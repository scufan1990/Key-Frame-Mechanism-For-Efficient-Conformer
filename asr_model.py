...
class ASRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.centor = None
        if self.centor_weight > 0.0:
            self.centor = torch.nn.Parameter(torch.zeros(vocab_size, decoder.embed_dim), requires_grad=True)
            self.criterion_centor = torch.nn.MSELoss(reduction='none')
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out, encoder_mask,ctc_loss_1 = self.encoder(speech, speech_lengths,text, text_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)

        # 2a. Attention-decoder branch
        if self.ctc_weight != 1.0:
            loss_att, acc_att, loss_centor = self._calc_att_loss(encoder_out, encoder_mask,
                                                                 text, text_lengths)
        else:
            loss_att, loss_centor = None, None
        ...
        if self.ctc_weight != 0.0:
            try:
                ...
                print("compute ctc loss err!")
        else:
            loss_ctc = None

        if self.sdc_weight > 0.0:
            loss_sdc = self._calc_sdc_loss(encoder_out)
        else:
            loss_sdc = None
        ...
        if loss_ctc is None:
            ...
            loss = self.ctc_weight * (loss_ctc * 0.7 +ctc_loss_1 * 0.3) + \
                   (1 - self.ctc_weight) * loss_att

        loss_log = {"loss": loss, "loss_att": loss_att, "loss_ctc": (loss_ctc * 0.7 + ctc_loss_1 * 0.3)}
        if loss_sdc is not None:
            loss += self.sdc_weight * loss_sdc
            loss_log["loss_sdc"] = loss_sdc

        if loss_centor is not None:
            loss += self.centor_weight * loss_centor
            loss_log["loss_centor"] = loss_centor
        return loss_log
    ...
    def _calc_att_loss(
        ...
        r_ys_in_pad, r_ys_out_pad = add_sos_eos(r_ys_pad, self.sos, self.eos,
                                                self.ignore_id)
        # 1. Forward decoder
        decoder_out, decoder_emb, r_decoder_out, r_decoder_emb, _ = self.decoder(
        ...
        loss_centor = torch.tensor(0.0)
        if self.centor_weight > 0:
            loss_centor = self._calc_centor_loss(decoder_emb, ys_out_pad)
        r_loss_att = torch.tensor(0.0)
        r_loss_centor = torch.tensor(0.0)
        if self.reverse_weight > 0.0:
            r_loss_att = self.criterion_att(r_decoder_out, r_ys_out_pad)
            if self.centor_weight > 0:
                r_loss_centor = self._calc_centor_loss(r_decoder_emb, r_ys_out_pad)
        loss_att = loss_att * (
            1 - self.reverse_weight) + r_loss_att * self.reverse_weight
        if self.centor_weight > 0.0:
            loss_centor = loss_centor * (
                1 - self.reverse_weight) + r_loss_centor * self.reverse_weight
        acc_att = th_accuracy(
            decoder_out.view(-1, self.vocab_size),
            ys_out_pad,
            ignore_label=self.ignore_id,
        )
        return loss_att, acc_att, loss_centor if self.centor_weight > 0.0 else None


    def _calc_centor_loss(
        self, 
        decoder_emb: torch.Tensor, 
        label: torch.Tensor) -> torch.Tensor:

        # compute centor loss
        B, L, D = decoder_emb.size()
        ignore = label == self.ignore_id
        targets = self.centor[label.reshape(-1)].reshape(B, L, D)
        loss_centor = self.criterion_centor(decoder_emb, targets)
        loss_centor = 0.5 * loss_centor.masked_fill(ignore.unsqueeze(2), 0).sum() / B
        return loss_centor
    ...
    def _calc_sdc_loss(
        ...
        loss_sdc = torch.std(encoder_out, dim=1)
        loss_sdc = loss_sdc.sum() / B / D
        return loss_sdc
    ...
    def recognize(
        ...
        encoder_out, encoder_mask = self._forward_encoder(
            speech, speech_lengths, target, target_lengths,decoding_chunk_size,
    ...
    def ctc_greedy_search(
        ...
        # Let's assume B = batch_size
        encoder_out, encoder_mask = self._forward_encoder(
            speech, speech_lengths, text, text_lens, decoding_chunk_size,
            num_decoding_left_chunks,
            simulate_streaming)  # (B, maxlen, encoder_dim)
        maxlen = encoder_out.size(1)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
    ...
    def attention_rescoring(
        ...
        hyps, encoder_out = self._ctc_prefix_beam_search(
            speech, speech_lengths, target, target_lengths, beam_size, decoding_chunk_size,
